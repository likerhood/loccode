...
from CoSIL.util.parse_global_var import parse_global_var_from_code
...
def correct_file_paths(model_found_files, files):
    ...
    try:
        from muladapter.path_resolver import resolve_file_candidates

        all_files = [file_content[0] for file_content in files]
        return resolve_file_candidates(model_found_files, all_files, limit=5)
    except Exception:
        found_files = []
