#!/usr/bin/env python3
"""CoSIL-local adapter for multilingual/multimodal benchmark inputs."""
...
def extract_symbols(path: str, content: str) -> list[dict]:
    ...
    for typ, raw in patterns:
        for m in re.finditer(raw, content, flags):
            name = m.group(1)
            key = (typ, name, m.start())
            if key in seen:
                continue
            seen.add(key)
            line = _line_no(offsets, m.start())
            snippet = "\n".join(content.splitlines()[max(0, line - 1): line + 20])
            symbols.append({"type": typ, "name": name, "path": path, "start_line": line, "end_line": line + len(snippet.splitlines()) - 1, "content": snippet})
    return symbols
...
def _insert_path(root: dict, parts: list[str], value: dict) -> None:
    cur = root
    for part in parts[:-1]:
...
def export_for_cosil(parsed: dict, samples: list[dict], output_dir: Path) -> None:
    structure = {}
    for item in parsed["files"]:
        classes = [{"name": s["name"], "start_line": s["start_line"], "end_line": s["end_line"], "text": s["content"]} for s in item["symbols"] if s["type"] == "class"]
        funcs = [{"name": s["name"], "start_line": s["start_line"], "end_line": s["end_line"], "text": s["content"]} for s in item["symbols"] if s["type"] != "class"]
        _insert_path(structure, item["path"].split("/"), {"text": item["content"], "classes": classes, "functions": funcs})
    for sample in samples:
        instance_id = sample.get("instance_id", "sample")
        write_json(output_dir / "repo_structures" / f"{instance_id}.json", {"structure": structure, "adapter": "muladapter"})
...
def command_smoke(args: argparse.Namespace) -> None:
    ...
    export_for_cosil(parsed, samples, Path(args.output_dir))
