...
from muladapter.adapter import detect_language, enrich_problem_statement, export_for_cosil, scan_repo
...
class MulAdapterTest(unittest.TestCase):
    ...
    def test_scan_and_export_two_samples(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            out = Path(td) / "out"
            export_for_cosil(parsed, samples, out)
            data = json.loads((out / "repo_structures" / "demo__1.json").read_text())
