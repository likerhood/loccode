...
ALLOW_NO_IMAGES="${ALLOW_NO_IMAGES:-0}"
ALLOW_NON_GALA_FILES="${ALLOW_NON_GALA_FILES:-0}"
...
TEST_NAME="${TEST_NAME:-omnigirl-js-ts-multimodal-60}"
...
echo "[1/6] Prepare OmniGIRL ${SAMPLE_SIZE}-sample localization subset"
PREPARE_ARGS=(
  --dataset "${DATASET}"
  --split "${SPLIT}"
...
if [[ "${ALLOW_NO_IMAGES}" == "1" ]]; then
  PREPARE_ARGS+=(--allow-no-images)
fi
if [[ "${ALLOW_NON_GALA_FILES}" == "1" ]]; then
  PREPARE_ARGS+=(--allow-non-gala-files)
fi
"${PYTHON_BIN}" "${SCRIPT_DIR}/prepare_omnigirl_js_ts_multimodal_60.py" "${PREPARE_ARGS[@]}"
...
echo
echo "Done."
echo "Relaxed metrics: ${EVAL_DIR}/metrics_3level.md"
