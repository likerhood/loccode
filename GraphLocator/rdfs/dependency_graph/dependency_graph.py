...
class DependencyGraph:
    ...
    def get_membership_parent(self, node: Node) -> Node:
        in_neighbors_with_relation = [
            n for n, target, edge_data in self.graph.in_edges(node, data=True)
            if self.graph[n][node][0]['relation'].relation == EdgeRelation.HasMember and n.location is not None
        ]
