...
def wrap_code_snippet(code_snippet, start_line, end_line):
    ...
    if not start_line == end_line == 1 and max_line_number != end_line:
        # Some lightweight multilingual indexes store best-effort ranges while
        # code snippets are later split with Python's exact "\n" semantics. For
        # display purposes, prefer the actual snippet length over crashing the
        # whole search tool on a trailing-newline mismatch.
        end_line = max_line_number
    ...
    def global_name_dict(self):
        if self._global_name_dict is None:  # Compute only once
            ...
            for nid in self.G.nodes():
                ...
                if self.G.nodes[nid].get('type') == NODE_TYPE_FILE:
                    fname = nid.split('/')[-1]
