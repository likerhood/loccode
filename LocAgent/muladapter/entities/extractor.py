...
def extract_entities_from_structure_file(file_path: str, file_node: dict[str, Any]) -> list[CodeEntity]:
    ...
    qualified_entities: list[CodeEntity] = []
    for entity in entities:
        if entity.kind == "method" and "." not in entity.qualified_name:
            parents = [
                cls for cls in classes
                if cls.start_line < entity.start_line <= cls.end_line
            ]
            if parents:
                parent = sorted(
                    parents,
                    key=lambda cls: (cls.end_line - cls.start_line, -cls.start_line),
                )[0]
                entity = CodeEntity(
                    entity.file,
                    entity.kind,
                    entity.name,
                    f"{parent.qualified_name}.{entity.name}",
                    entity.start_line,
                    entity.end_line,
                    entity.language,
                )
    ...
    return sorted(qualified_entities, key=lambda item: (item.start_line, item.kind, item.name))
