...
import subprocess
import logging
from muladapter.query_fallback import empty_search_hint, fallback_search_terms
from muladapter.search_defaults import default_search_pattern, normalize_repo_path, select_source_files
...
def _limit_query_results(query_results):
    max_results = _env_int("LOCAGENT_MAX_SEARCH_RESULTS", 8)
    if max_results <= 0:
        return query_results
    return query_results[:max_results]


def normalize_file_pattern(file_path_or_pattern: str | None) -> str | None:
    if file_path_or_pattern is None:
        return None
    pattern = str(file_path_or_pattern).strip()
    changed = True
    while changed and len(pattern) >= 2:
        changed = False
        for quote in ("'", '"', "`"):
            if pattern.startswith(quote) and pattern.endswith(quote):
...
def _repo_dirs(all_file_paths: list[str]) -> set[str]:
    dirs: set[str] = set()
    for file_path in all_file_paths:
        parts = normalize_repo_path(file_path).split("/")[:-1]
        for end in range(1, len(parts) + 1):
            dirs.add("/".join(parts[:end]))
    return dirs


def suggest_similar_file_pattern(
        file_path_or_pattern: str | None,
        all_file_paths: list[str],
) -> str | None:
    pattern = normalize_file_pattern(file_path_or_pattern)
    if not pattern:
        return None

    static_prefix = re.split(r"[*?\[]", pattern, maxsplit=1)[0].rstrip("/")
    if not static_prefix or static_prefix in {".", "/"}:
        return None

    dirs = _repo_dirs(all_file_paths)
    if static_prefix in dirs:
        return None

    parent = static_prefix.rsplit("/", 1)[0] if "/" in static_prefix else ""
    nearby_dirs = [
        directory for directory in dirs
        if not parent or directory.startswith(parent + "/")
    ]
    matches = difflib.get_close_matches(static_prefix, nearby_dirs or list(dirs), n=1, cutoff=0.72)
    if not matches:
        target_name = static_prefix.rsplit("/", 1)[-1]
        name_matches = [
            directory for directory in nearby_dirs
            if difflib.SequenceMatcher(None, target_name, directory.rsplit("/", 1)[-1]).ratio() >= 0.72
        ]
        matches = sorted(name_matches, key=len)[:1]
    if not matches:
        return None

    suffix = "**/*"
    extension_match = re.search(r"\*\*?/\*?(\.[A-Za-z0-9]+)$", pattern)
    if extension_match:
        suffix = f"**/*{extension_match.group(1)}"
    return f"{matches[0].rstrip('/')}/{suffix}"
...
CURRENT_STRUCTURE_INDEX: StructureIndex | None = None
...
def _graph_has_file_nodes(G: nx.MultiDiGraph) -> bool:
    return any(data.get("type") == NODE_TYPE_FILE for _, data in G.nodes(data=True))


def _load_or_build_multilang_graph(dataset: str, split: str, rank: int) -> nx.MultiDiGraph:
    graph_index_file = f"{GRAPH_INDEX_DIR}/{CURRENT_ISSUE_ID}.multilang.pkl"
    if os.path.exists(graph_index_file):
        logging.info("[%s] Loading multilingual graph fallback for %s", rank, CURRENT_ISSUE_ID)
    ...
    G = build_multilang_graph(repo_dir)
    with open(graph_index_file, "wb") as f:
        pickle.dump(G, f)
    logging.info("[%s] Built multilingual graph fallback for %s", rank, CURRENT_ISSUE_ID)
    return G
...
def set_current_issue(instance_id: str = None, 
    ...
    global DP_GRAPH_ENTITY_SEARCHER, DP_GRAPH_DEPENDENCY_SEARCHER, DP_GRAPH, CURRENT_STRUCTURE_INDEX
    ...
    if not _graph_has_file_nodes(G):
        logging.warning(
            "[%s] Graph index for %s has no file nodes; using multilingual graph fallback.",
            rank,
            CURRENT_ISSUE_ID,
        )
        try:
            G = _load_or_build_multilang_graph(dataset, split, rank)
        except Exception as exc:
            logging.error(
                "[%s] Failed to build multilingual graph fallback for %s: %s",
                rank,
                CURRENT_ISSUE_ID,
                exc,
            )

    CURRENT_STRUCTURE_INDEX = load_structure_index(CURRENT_ISSUE_ID)
    if CURRENT_STRUCTURE_INDEX:
        logging.info(
            "[%s] Loaded repo structure index for %s: files=%s symbols=%s",
            rank,
            CURRENT_ISSUE_ID,
            len(CURRENT_STRUCTURE_INDEX.list_files()),
            sum(len(CURRENT_STRUCTURE_INDEX.entities_for_file(path)) for path in CURRENT_STRUCTURE_INDEX.list_files()),
        )
...
def reset_current_issue():
    ...
    global CURRENT_STRUCTURE_INDEX
...
def get_current_structure_index() -> StructureIndex | None:
    global CURRENT_STRUCTURE_INDEX
    return CURRENT_STRUCTURE_INDEX


def _structure_file_paths() -> list[str]:
    structure_index = get_current_structure_index()
    return structure_index.list_files() if structure_index else []


def _format_structure_scope_hint(include_files: list[str], limit: int = 8) -> str:
    structure_index = get_current_structure_index()
    if not structure_index or not include_files:
        return ""
    summaries = []
    for file_path in include_files[:limit]:
        resolved = structure_index.resolve_file(file_path)
        if resolved:
            summaries.append(structure_index.format_file_summary(resolved, max_symbols=20))
    if not summaries:
        return ""
    return (
        "\n\n### Repository structure symbol hints\n"
        "Use these exact function/class names for final localization when relevant:\n"
        + "\n\n".join(summaries)
        + "\n"
    )


def _format_structure_keyword_hint(
    ...
    include_set = {
        ...
        if (resolved := structure_index.resolve_file(file_path))
    } or set(structure_index.list_files())
    terms = [
        term.lower()
    ...
    def score_file(file_path: str) -> tuple[int, int, str]:
        symbols = structure_index.entities_for_file(file_path)
        haystack = " ".join(
            [file_path] + [entity.qualified_name for entity in symbols]
        ).lower()
        overlap = sum(1 for term in terms if term in haystack)
        symbol_bonus = min(len(symbols), 10)
        return (overlap, symbol_bonus, file_path)

    ranked = sorted(include_set, key=score_file, reverse=True)
    ranked = [
        file_path
        for file_path in ranked
        if score_file(file_path)[0] > 0
    ] or ranked
    summaries = [
        structure_index.format_file_summary(file_path, max_symbols=20)
        for file_path in ranked[:limit]
    ]
    if not summaries:
        return ""
    return (
        "\n\n### Lightweight repository structure search results\n"
        "The search call had no explicit query, so LocAgent avoided a broad BM25 scan. "
        "Use these exact file/function/class names if relevant, or call search_code_snippets "
        "again with concrete search_terms for deeper code snippets:\n"
        + "\n\n".join(summaries)
        + "\n"
    )
...
def search_code_snippets(
    ...
    Args:
        ...
        file_path_or_pattern (Optional[str]): A glob pattern or specific file path used to filter search results
            to particular files or directories. When omitted, LocAgent selects source files across Python,
            JavaScript, TypeScript, JSX/TSX, Java, and other common languages.
    ...
    if not all_file_paths:
        all_file_paths = _structure_file_paths()

    result = ""
    # exclude_files = find_matching_files_from_list(all_file_paths, "**/test*/**")
    empty_search_call = not search_terms and not line_nums
    if empty_search_call:
        search_terms = fallback_search_terms(get_current_issue_data())
        result += empty_search_hint(get_current_issue_data())

    original_file_path_or_pattern = file_path_or_pattern
    file_path_or_pattern = normalize_file_pattern(file_path_or_pattern)

    if file_path_or_pattern:
        if original_file_path_or_pattern != file_path_or_pattern:
            result += (
                f"Normalized file pattern from {original_file_path_or_pattern!r} "
                f"to '{file_path_or_pattern}'.\n"
            )
        include_files = find_matching_files_from_list(all_file_paths, file_path_or_pattern)
        if not include_files:
            suggested_pattern = suggest_similar_file_pattern(file_path_or_pattern, all_file_paths)
            if suggested_pattern:
                suggested_files = find_matching_files_from_list(all_file_paths, suggested_pattern)
                if suggested_files:
                    include_files = suggested_files
                    result += (
                        f"No files found for file pattern '{file_path_or_pattern}'. "
                        f"Using nearest existing file pattern '{suggested_pattern}' instead.\n"
                    )
                    file_path_or_pattern = suggested_pattern
            if not include_files:
                include_files = select_source_files(all_file_paths)
                result += (
                    f"No files found for file pattern '{file_path_or_pattern}'. "
                    "Will search multi-language source files instead.\n...\n"
                )
    else:
        file_path_or_pattern = default_search_pattern(all_file_paths)
        include_files = select_source_files(all_file_paths, file_path_or_pattern)
        result += f"Using multi-language default file pattern '{file_path_or_pattern}'.\n"

    if not search_terms and not line_nums:
        return result

    if empty_search_call:
        result += _format_structure_keyword_hint(search_terms or [], include_files)
        return result.strip()
    ...
    for query_infos, format_to_results in ranked_query_to_results.items():
        ...
        for format_mode, query_results in format_to_results.items():
            query_results = _limit_query_results(query_results)
            if format_mode == 'fold':
                cur_retrieve_src = ''
                for qr in query_results:
    ...
    if not all_query_results and include_files:
        result += _format_structure_scope_hint(include_files)
    elif include_files:
        result += _format_structure_scope_hint(include_files[:3], limit=3)
...
def get_entity_contents(entity_names: List[str]):
    ...
    for name in entity_names:
        ...
        if searcher.has_node(name):
            ...
            structure_match = structure_index.resolve_graph_id(name) if structure_index else None
            if structure_match:
                result += "Exact match found in repository structure fallback.\n"
                result += structure_index.format_entity(structure_match, include_code=True)
                result += '\n\n'
                continue
            suggestions = suggest_entity_names(name, searcher)
            result += 'Invalid name. \nHint: Valid entity name should be formatted as "file_path:QualifiedName" or just "file_path".'
            if suggestions:
                result += '\nSimilar valid entity names found in this repository:\n'
                result += '\n'.join([f'- {suggestion}' for suggestion in suggestions])
                result += '\nPlease call get_entity_contents again with one of these exact names if it is relevant.'
            result += '\n\n'
    return result.strip()


def suggest_entity_names(name: str, searcher: RepoEntitySearcher, limit: int = 5) -> List[str]:
    """Return nearby valid entity names when the model guesses an invalid path."""
    raw_terms = [
        name,
    ...
    if not all_file_paths:
        all_file_paths = _structure_file_paths()
    for term in terms:
        for file_path in find_matching_files_from_list(all_file_paths, term):
    ...
    structure_index = get_current_structure_index()
    if structure_index:
        for entity in structure_index.iter_entities():
            if len(suggestions) >= limit:
                return suggestions
            haystack = f"{entity.file}:{entity.qualified_name}"
            if any(term and term.lower() in haystack.lower() for term in terms):
                if haystack not in seen:
                    seen.add(haystack)
                    suggestions.append(haystack)

    try:
        nids = fuzzy_retrieve(name, graph=get_graph(), similarity_top_k=limit)
    except Exception as exc:
        logger.warning("Failed to fuzzy suggest entity names for `%s`: %s", name, exc)
        return suggestions

    for nid in nids:
        if searcher.has_node(nid) and nid not in seen:
            seen.add(nid)
...
def bm25_module_retrieve(
    ...
    similarity_top_k = min(similarity_top_k, _env_int("LOCAGENT_BM25_TOP_K", 8))
    ...
    try:
        ...
        logging.warning(
            f'{e}. Probably because BM25 top-k exceeds the available candidate count for query `{query}`.'
        )
        return []
...
def bm25_content_retrieve(
    ...
    try:
        ...
        logging.warning(
            "BM25 content index is unavailable for %s query `%s`; "
            "falling back to repository structure hints. Error: %s",
    ...
    for node in retrieved_nodes:
        ...
        span_ids = node.metadata.get('span_ids') or []
        # print(node.metadata)
        if not include_files or file in include_files:
            if 'text_fallback' in span_ids:
                query_result = QueryResult(query_info=query_info,
                                           format_mode='code_snippet',
                                           ...
                                           retrieve_src=f"Retrieved code content using text fallback keyword search (bm25)."
                                           )
                cur_query_results.append(query_result)
                continue
            ...
            #     continue
            if all([span_id in ['docstring', 'imports', 'comments'] for span_id in span_ids]):
                # TODO: drop ?
                query_result = QueryResult(query_info=query_info, 
                                           format_mode='code_snippet',
                                           nid=node.metadata['file_path'],
            ...
            elif any([span_id in ['docstring', 'imports', 'comments'] for span_id in span_ids]):
                nids = []
