...
Tool-use rules:
- Never call `search_code_snippets` with empty arguments.
- Always provide concrete `search_terms` or `line_nums`.
- For JavaScript/TypeScript/frontend repositories, use a multi-language pattern such as `file_path_or_pattern="**/*"` or a narrower path like `client/**/*`.
"""
