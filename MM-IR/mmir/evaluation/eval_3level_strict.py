...
def _strict_acc(preds: list[str], gold: set[str], cutoff: int) -> bool:
    return bool(gold) and gold <= set(preds[:cutoff])
    ...
    *,
    ...
    for level, rows in per_level_data.items():
        ...
        for cutoff in CUTOFFS:
            ranking[f"acc@{cutoff}"] = _pct(sum(1 for gold, preds in rows if _strict_acc(preds, gold, cutoff)) / max(1, len(rows)))
        reciprocal = []
        average_precisions = []
