...
def parse_log_pytest_v3(log: str) -> dict[str, str]:
    ...
    # Flag to indicate when to start processing the log
    process_log = False
    ...
    for line in log.split("\n"):
        if "=========================== short test summary info ============================" in line:
         ...
         # Only process lines after the marker
        if process_log:    
            if any([line.startswith(x.value) for x in TestStatus]):
                # Additional parsing for FAILED status
                if line.startswith(TestStatus.FAILED.value):
...
def parse_log_dateutil_v1(log: str) -> dict[str, str]:
    """
    Parser for test logs generated with PyTest framework, capturing the entire line after status.

    Args:
        log (str): log content
    Returns:
        dict: test case to full log line mapping
    """
    test_status_map = {}
    ...
    for line in log.split("\n"):
        line = line.strip()
        if " ... " in line:
            prev_test = line.split(" ... ")[0].strip()
        if line.endswith("... FAIL") :
            test_case = line.split("... FAIL")
            # print(test_case)
            # input()
            test_status_map[test_case[0].strip()] = 'FAILED'
        elif line.startswith("FAIL: "):
            test = line.replace("FAIL: ",'').strip()
            test_status_map[test] = 'FAILED'
        elif line.endswith("... ERROR"): 
            test_case = line.split("... ERROR")
            test_status_map[test_case[0].strip()] = 'ERROR'
        elif line.startswith("ERROR:"):
            test = line.replace("ERROR: ",'').strip()
            test_status_map[test] = 'ERROR'
            
        elif line.endswith("... ok"):
            test_case = line.split("... ok")
            test_status_map[test_case[0].strip()] = 'PASSED'
        elif  "... skipped " in line:
            test_case = line.split('... skipped')
            test_status_map[test_case[0].strip()] = 'SKIPPED'
        if line.lstrip().startswith("ok") and prev_test is not None:
            # It means the test passed, but there's some additional output (including new lines)
            # between "..." and "ok" message
            test = prev_test
...
def parse_log_jest_v1(log: str) -> dict[str, str]:
    """
    Parser for test logs generated with PyTest framework, capturing the entire line after status.
    ...
    for line in log.split("\n"):
        ...
        if line.startswith('PASS '):
            # print(line)
            # input()
            test_suite = line.split()[1].strip()
            # test_suite = re.sub(pattern, '', test_suite).strip()
            test_status_map[test_suite] = 'PASSED'
        if line.startswith('FAIL '):
            test_suite = line.split()[1].strip()
            # test_suite = re.sub(pattern, '', test_suite)
            test_status_map[test_suite] = 'FAILED'
...
def parse_log_dayjs_v1(log: str) -> dict[str, str]:
    """
    Parser for test logs generated with PyTest framework, capturing the entire line after status.
    ...
    for line in log.split("\n"):
        ...
        if line.startswith('PASS '):
            ...
            # input()
            test_suite = line.split()[1].strip()
            if test_suite == 'test/timezone.test.js':
                test_suite = f'{test_suite}({timezone_test_num})'
                timezone_test_num += 1
            # test_suite = re.sub(pattern, '', test_suite).strip()
            test_status_map[test_suite] = 'PASSED'
        if line.startswith('FAIL '):
...
def parse_log_assertj(log: str) -> dict[str, str]:
    test_status_map = {}

    log_line_list = log.split('\n')
    for line in log_line_list:
        if 'Tests run:' in line and 'Time elapsed:' in line:
            test_name = 'org.'+line.split('- in ')[1].strip()
            errors_pattern = r"Errors: (\d+)"
            errors_match = re.search(errors_pattern, line)
            errors = None
            if errors_match:
            ...
            failures_match = re.search(failures_pattern, line)
            failures = None
            if failures_match:
                failures = int(failures_match.group(1))
                if failures>0:
                    test_status_map[test_name] = 'FAILED'
            if errors != None and failures !=None:
...
parse_log_mypy = parse_log_pytest_v3
parse_log_statsmodels = parse_log_pytest_v3
parse_log_dateutil = parse_log_pytest_v3
parse_log_dateutil_old = parse_log_dateutil_v1
parse_log_redis_py= parse_log_pytest_v3
parse_log_tqdm = parse_log_pytest_v3
parse_log_cryptography = parse_log_pytest_v3

parse_log_jest =parse_log_jest_v1
parse_log_webpack = parse_log_jest_v1
parse_log_babel = parse_log_jest_v1
parse_log_tailwindcss = parse_log_jest_v1
parse_log_prettier= parse_log_jest_v1
parse_log_dayjs = parse_log_dayjs_v1

parse_log_assertj = parse_log_assertj
parse_log_gson = parse_log_assertj
parse_log_netty = parse_log_assertj
...
MAP_REPO_TO_PARSER = {
    ...
    'dateutil/dateutil' : parse_log_dateutil,
    'dateutil/dateutil-old' : parse_log_dateutil_old,
    'redis/redis-py' : parse_log_redis_py,
    'tqdm/tqdm' :   parse_log_tqdm,
    'pyca/cryptography' : parse_log_cryptography,

    'prettier/prettier':parse_log_prettier,
    'webpack/webpack':parse_log_webpack,
    'tailwindlabs/tailwindcss':parse_log_tailwindcss,
    'babel/babel':parse_log_babel,
    'jestjs/jest':parse_log_jest,
    'iamkun/dayjs':parse_log_dayjs,

    'assertj/assertj':parse_log_assertj,
    'google/gson':parse_log_gson,
    'netty/netty':parse_log_netty
}
