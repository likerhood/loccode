#!/usr/bin/env python3
"""OrcaLoca-local adapter for multilingual/multimodal benchmark inputs."""
...
def export_for_orcaloca(parsed: dict, samples: list[dict], output_dir: Path) -> None:
    snippets = []
    for item in parsed["files"]:
        snippets.append({"path": item["path"], "language": item["language"], "kind": "file", "name": item["path"], "text": item["content"][:8000]})
        for symbol in item["symbols"]:
            snippets.append({"path": symbol["path"], "language": item["language"], "kind": symbol["type"], "name": symbol["name"], "start_line": symbol["start_line"], "end_line": symbol["end_line"], "text": symbol["content"]})
    for sample in samples:
        instance_id = sample.get("instance_id", "sample")
        write_json(output_dir / "search_cache" / f"{instance_id}.json", {"snippets": snippets, "adapter": "muladapter"})
...
def command_smoke(args: argparse.Namespace) -> None:
    ...
    export_for_orcaloca(parsed, samples, Path(args.output_dir))
