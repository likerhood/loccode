...
from muladapter.adapter import detect_language, enrich_problem_statement, export_for_orcaloca, scan_repo
...
class MulAdapterTest(unittest.TestCase):
    ...
    def test_scan_and_export_two_samples(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            out = Path(td) / "out"
            export_for_orcaloca(parsed, samples, out)
            data = json.loads((out / "search_cache" / "demo__1.json").read_text())
            self.assertGreaterEqual(len(data["snippets"]), 3)
            self.assertTrue((out / "data" / "samples_enriched.jsonl").exists())
