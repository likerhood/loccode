...
def select_rows(rows: list[dict[str, Any]], sample_size: int, seed: int) -> list[dict[str, Any]]:
    candidates = [
        ...
        and row.get("files") and (row.get("image_urls") or row.get("web_urls"))
    ]
...
def main() -> None:
    ...
    selected = select_rows(normalized, args.sample_size, args.seed)
    print(
    ...
    summary = {
        ...
        "candidate_rows": len(select_rows(normalized, 0, args.seed)),
        "seed": args.seed,
